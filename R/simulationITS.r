...
simulateITSMenchetti <- function(n, timePeriods, interventionTime = timePeriods-1, linear = TRUE){
    ...
    xF1 <- rep(sample(c(0,1), n, replace = T), each = timePeriods) 
    xF2 <- rep(sample(c(0,1), n, replace = T), each = timePeriods)
    ...
    df <- cbind(time, id, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, xF1, xF2)
    df <- cbind(df, y = 0)
    df <- as.data.frame(df)
    ...
    df$y_obs <- df$y + 2*df$int + u_i*df$xF1 + 0.5*u_i*df$xF2
...
df <- simulateITSMenchetti(n = 20, timePeriods = 36, interventionTime = 30)
...
STEPS = 3
covariates_time = NULL
covariates_var = c("x1", "x2")
covariates_fix = c("xF1", "xF2")
key = "id"
y = "y"
method = c("lm", "rf", "xgboost")
K = 5
