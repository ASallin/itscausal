test_that("forecastITS works", {
  ...
  f1 <- forecastITS(
    ...
    time = "time", INDEX = 25, covariates_time = c("year"), key = "id",
    y = "y",
    covariates_fix = c("x1", "x2"),
    covariates_var = c("xv1", "xv2")
  )

  # Unbalanced panel — INDEX=13 places the year boundary (t=12→t=13) between
  # the two auto-computed training positions, ensuring year variation in the
  # variation check.
  df <- smallSimulation(n = 10, t = 25, index = 13)
  dfUB <- df[sample(1:250, 230), ]
  fUB <- forecastITS(
    data = dfUB, time = "time", INDEX = 13, covariates_time = c("year"), key = "id",
    y = "y"
  )
