...
df$y <- with(
  ...
  constant + param_timetrend * abs_time
    ...
    + param_slopeintervention * post * abs_time # slope after the cutoff
    + season_effect
    + error
  #+ 0.2 * X
...
lm(
  y ~ abs_time + post + abs_time * post + as.factor(season),
  data = df
)
...
df %>%
  group_by(time, post) %>%
  summarise(y = mean(y), model = mean(model), forecast = mean(forecast)) %>%
  # mutate(y = ifelse(post == 1, NA, y)) |>
  ggplot(aes(y = y, x = time)) +
  geom_line(linetype = 2, size = 0.5) +
  geom_line(aes(x = time, y = model), col = "red") +
  geom_line(
    aes(
      x = time,
      y = ifelse(time < 1, NA,
